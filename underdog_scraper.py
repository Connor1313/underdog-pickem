...
class UnderdogScraper:
    ...
    def apply_name_corrections(self, df):
        name_corrections = {
            # ... Use this dictionary to match names if needed
        }
    ...
    def process_data(self, players, appearances, over_under_lines):
        ...
        over_under_lines_expanded = pd.concat(
            [over_under_lines_expanded.drop("options", axis=1).reset_index(drop=True),
             options_df.reset_index(drop=True)], axis=1
        )

        over_under_lines_expanded["appearance_id"] = over_under_lines_expanded[
            "over_under"
        ].apply(lambda x: x["appearance_stat"]["appearance_id"])
        over_under_lines_expanded["stat_name"] = over_under_lines_expanded[
            "over_under"
        ].apply(lambda x: x["appearance_stat"]["stat"])
        ...
        ).fillna(over_under_lines_expanded["choice"])

        underdog_props = player_appearances.merge(
            over_under_lines_expanded,
            on="appearance_id",
            how="left",
            suffixes=("", "_over_under")
        )
    ...
    def filter_data(self, df):
        esports_ids = ["LOL", "ESPORTS", "VALORANT"]  # Replace with actual esports IDs
    ...
    def scrape(self):
        ...
        self.underdog_props["full_name"] = (
            self.underdog_props["first_name"].fillna("") + " " + self.underdog_props["last_name"].fillna("")
        )
        self.underdog_props["full_name"] = self.underdog_props["full_name"].str.strip()
        ...
        columns_to_keep = [
            ...
            "american_price",       # Betting odds in American format
            "decimal_price"         # Betting odds in Decimal format
        ]
        self.underdog_props = self.underdog_props[columns_to_keep]

        # Save as a CSV file instead of an XLSX file.
        csv_filename = "underdog_esports_data.csv"
        self.underdog_props.to_csv(csv_filename, index=False)
        print(f"Esports data saved to {csv_filename}")
